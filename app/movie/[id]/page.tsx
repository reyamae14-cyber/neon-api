...
export default function MoviePage() {
  ...
  const movieServers: ServerOption[] = [
    ...
    {
      name: "SamXerz",
      url: "https://player.vidplus.to/embed/movie/{id}",
    },
    { name: "Icarus", url: "https://vidfast.pro/movie/" },
    { name: "Orion", url: "https://player.vidsrc.co/embed/movie/" },
    {
      name: "Theseus",
      url: "https://player.vidify.top/embed/movie/{id}?autoplay=true&poster=true&chromecast=true&servericon=true&setting=true&pip=true&download=true&logourl=https%3A%2F%2Fzetflix-tv.vercel.app%2Fzetflix.svg&font=Roboto&fontcolor=6f63ff&fontsize=20&opacity=0.5&primarycolor=00FFFF&secondarycolor=FF00FF&iconcolor=00FFFF",
    },
  ]
  ...
  useEffect(() => {
    ...
    return unsubscribe
  }, []) // Removed currentUrl dependency to prevent loop

  const refreshPings = async () => {
    const updatedServers = await Promise.all(
      movieServers.map(async (server) => {
  ...
  const loadContent = (serverUrl: string) => {
    ...
    if (serverUrl.includes("watch.vidora.su")) {
      // Zeticuz
      fullUrl = `${serverUrl}${movieId}?autoplay=true&colour=${themeColor}&backbutton=https%3A%2F%2Fzetflix-tv.vercel.app&logo=https%3A%2F%2Fzetflix-tv.vercel.app%2Fzetflix-logo.png&pausescreen=true&autonextepisode=true`
    } else if (serverUrl.includes("xprime.tv")) {
      // Infested
      fullUrl = `${serverUrl}${movieId}?autoplay=true`
    } else if (serverUrl.includes("hexa.watch")) {
      // Spectre
      fullUrl = `${serverUrl}${movieId}?autoplay=true&colour=${themeColor}&pausescreen=true&autonextepisode=true`
      ...
      // SamXerz
      fullUrl =
        serverUrl.replace("{id}", movieId) +
        `?autoplay=true&primarycolor=${primaryColor}&secondarycolor=${secondaryColor}&iconcolor=FFFFFF&poster=true&title=true&chromecast=true&icons=netflix&font=Poppins&fontcolor=FFFFFF&fontsize=20&opacity=0.8&logourl=https%3A%2F%2Fzetflix-tv.vercel.app%2Fzetflix.svg`
    } else if (serverUrl.includes("player.vidify.top")) {
      // Theseus
      fullUrl = serverUrl.replace("{id}", movieId)
    } else {
      // Default fallback
      fullUrl = `${serverUrl}${movieId}`
    }
  ...
  useEffect(() => {
    loadContent(movieServers[0].url)
  }, [movieId])
